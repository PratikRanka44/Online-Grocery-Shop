...
from .models import ORDERSTATUS, Booking, Carousel, Category, Feedback, Product, UserProfile, Cart
...
def registration(request):
    if request.method == "POST":
        ...
        # Regular expression to validate that the email starts with a letter
        email_regex = r'^[A-Za-z][\w\.-]+@[A-Za-z]+\.[A-Za-z]{2,}$'

        # Check if the email is valid
        if not re.match(email_regex, email):
            messages.error(request, "Please enter a valid email address starting with a letter.")
        ...
        try:
            ...
            return redirect('userlogin')  # Redirect to the login page after registration
...
def home(request):
    ...
    for category in categories:
        products = Product.objects.filter(category=category)[:5]  # Get 5 products from each category
        if products.exists():  # Ensure there are products in this category
